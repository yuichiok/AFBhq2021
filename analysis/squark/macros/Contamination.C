...
//---------------
    ...
    if(isel==9) leg_true->SetHeader("reco-level, cheat on K-ID, "+KID);
    leg_true->SetTextSize(0.035);
    for(int q=1; q<6; q++) leg_true->AddEntry(h_AFB_true[q],TString::Format("%s, %s",quark_string[q].Data(),pol.Data()),"fl");
    ...
    c_eff_MC_true->Print(TString::Format("c_cont_true_MC_sel%i_%s_%s.eps",isel,KID.Data(),pol.Data()));
//--------------------------------
    ...
    if(isel==9) leg_reco->SetHeader("reco-level, cheat on K-ID, "+KID);
    leg_reco->SetTextSize(0.035);
    for(int q=1; q<6; q++) leg_reco->AddEntry(h_AFB_reco[q],TString::Format("%s, %s",quark_string[q].Data(),pol.Data()),"fl");
  ...
  //c_eff_MC->Print(TString::Format("summaryplots/UnEff_sample_%i_pdg_%i_twopols.pdf",sample,iquark));
    c_eff_MC_reco->Print(TString::Format("c_cont_reco_MC_sel%i_%s_%s.eps",isel,KID.Data(),pol.Data()));
