...
void plotsC2(int ipol = 2, int cuts = 0, float lum = 900, bool MC = false, bool Yield = true)
{
  ...
  if (Yield == false)
  {
	...
	h2_bkg.at(j).at(k)->GetYaxis()->SetRangeUser(-3,3);
	...
	h2_bkg.at(j).at(k)->GetYaxis()->SetRangeUser(-3,3);
        h2_bkg.at(j).at(k)->GetZaxis()->SetTitle("C^{(2)}(#Delta y, #Delta #phi)");
  ...
  else
  {
    ...
    for (int j = 0; j < h2_bkg.size(); j++)
    {
      ...
      for (int i = 0; i < 2; i++)
      {
        ...
        if (i == 0)
          leg->AddEntry(htemp[j][i], "0<|#Delta y|<1.6");
        if (i == 1)
          leg->AddEntry(htemp[j][i], "1.6<|#Delta y|<3");
...
void plotsC2LCWS2023(int ipol = 2, int cuts = 0, float lum = 900, bool MC = false, bool Yield = true)
{
...
void C2()
{
  ...
  cout << cuts << " ";
  plotsC2LCWS2023(pol, cuts, 900, true, false);
}
