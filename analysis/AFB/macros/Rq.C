...
TH1F* N_PQ_HistoBKG(TString histo="Nacc_Kc", int pol=0, int iquark=5,  float lum=900, float error=0.0, bool ilc500=false){
  ...
  //ttbar
  if(ilc500) hb0[4]=GetHisto(5,TString::Format("h_%s_0",histo.Data()),pol,iquark,lum,factor);
  
  //here
  hb0[0]->Add(hb0[1]);
  hb0[0]->Add(hb0[2]);
  hb0[0]->Add(hb0[3]);
  if(ilc500) hb0[0]->Add(hb0[4]);

  if(error!=0) {
    hb0[0]->Scale(fabs(error));
    hBeforeSelection->Add(hb0[0],error/fabs(error));
  }

  return hBeforeSelection;


}


TH1F* N0HistoBKG(int pol=0, int iquark=5,  float lum=900, float error=0.1, bool ilc500=false){

  int ipdgquark=0;
  if(iquark==5) ipdgquark=0;
  if(iquark==4) ipdgquark=1;
  TH1F *hBeforeSelection[4];
  //for(int i=0; i<1; i++) 
  hBeforeSelection[0] =GetHisto(1,TString::Format("h_Ntotal_nocuts_%i",ipdgquark),pol,iquark,lum,1);
 ...
 //ttbar
  if(ilc500) hb0[4]=GetHisto(5,TString::Format("h_N%i_0",0),pol,iquark,lum,factor);
  ...
  if(error!=0) {
    hb0[0]->Scale(fabs(error));
    hBeforeSelection[0]->Add(hb0[0],error/fabs(error));
  }
  ...
  TH1F *hsignal[3];
  for(int i=0; i<1; i++)
  hsignal[i] =GetHisto(1,TString::Format("h_N0_%i",ipdgquark),pol,iquark,lum,1);
...
TH1F* FHistoBKG(int pol=0, int histo=1, int iquark=5,  float lum=900, float error=0.1, bool ilc500=false){
  ...
  if(ilc500) hbj[4]=GetHisto(5,TString::Format("h_N%i_0",histo),pol,iquark,lum,1);

  hbj[0]->Add(hbj[1]);
  hbj[0]->Add(hbj[2]);
  hbj[0]->Add(hbj[3]);
  if(ilc500) hbj[0]->Add(hbj[4]);


  if(error!=0) hbj[0]->Scale(fabs(error));
  ...
  h0[0]->Add(h0[1]);
  h0[0]->Add(h0[2]);
  if(error!=0)  h0[0]->Add(hb0[0],error/fabs(error));

  hj[0]->Add(hj[1]);
  hj[0]->Add(hj[2]);
  if(error!=0)  hj[0]->Add(hbj[0],error/fabs(error));
...
TH1F* FHistoEffErr(int pol=0, int histo=1, int iquark=5,  float lum=900, float error_b=0, float error_uds=0){
  ...
  for(int i=0; i<h0[0]->GetNbinsX()+1; i++) {
    ...
    float epsilon_b_prime=0;
    if(histo==1) epsilon_b_prime = (effb*error_b);
    if(histo==2) epsilon_b_prime = (2.*effb*(effb*error_b));
    ...
    h1[1]->SetBinContent(i, h1[1]->GetBinContent(i) + h0[1]->GetBinContent(i)*(epsilon_b_prime));
    h2[1]->SetBinContent(i, h2[1]->GetBinContent(i) + h0[1]->GetBinContent(i)*(epsilon_b_prime));
    //SetBinError(i,sqrt( pow(h2[1]->GetBinError(i),2)+ pow(sqrt(h1[1]->GetBinContent(i) + h0[1]->GetBinContent(i)*epsilon_b),2)));
    ...
    float epsilon_uds_prime=0;
    if(histo==1) epsilon_uds_prime = (effuds*error_uds);
    if(histo==2) epsilon_uds_prime = (2.*effuds*(effuds*error_uds));
    h1[0]->SetBinError(i,sqrt( pow(h1[0]->GetBinError(i),2)+ pow(sqrt(h1[0]->GetBinContent(i) + h0[0]->GetBinContent(i)*epsilon_uds),2)));
    h2[0]->SetBinError(i,sqrt( pow(h2[0]->GetBinError(i),2)+ pow(sqrt(h1[0]->GetBinContent(i) + h0[0]->GetBinContent(i)*epsilon_uds),2)));
    h1[0]->SetBinContent(i, h1[0]->GetBinContent(i) + h0[0]->GetBinContent(i)*(epsilon_uds_prime));
    h2[0]->SetBinContent(i, h2[0]->GetBinContent(i) + h0[0]->GetBinContent(i)*(epsilon_uds_prime));
  }
...
TH1F* FHistoChargeBKG(TString method="Kc",int pol=0, int histo=1, int iquark=5,  float lum=900, float error=0, bool ilc500=false){
  ...
  if(ilc500)hb0[6]=GetHisto(5,TString::Format("h_N%i_%s_%i",0,method.Data(),0),pol,iquark,lum,factor);

  int imax=6;
  if(ilc500) imax=7;
  for(int i=1; i<imax; i++) hb0[0]->Add(hb0[i]);
  if(error!=0) {
    hb0[0]->Scale(fabs(error));
  ...
  if(ilc500)  hbj[6]=GetHisto(5,TString::Format("h_N%i_%s_%i",histo,method.Data(),0),pol,iquark,lum,1);
  for(int i=1; i<imax; i++) hbj[0]->Add(hbj[i]);
  if(error!=0) {
    hbj[0]->Scale(fabs(error));
    hj[0]->Add(hbj[0],error/fabs(error));
  }
