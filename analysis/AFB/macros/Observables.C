...
void Plots_CrossDiff(int mode=0) {
  ...
  c_AFBb->cd(1);
  // gPad->SetLogy();
  gAFBb->GetXaxis()->SetTitle("cos(#theta_{thrust})");
  ...
  Labels(", sin(#Psi_{acol})<0.3 & K_{#gamma}<35 GeV");
...
void Plots_R(int mode=0) {
  ...
  TLegend *leg_b = new TLegend(0.2,0.5,0.5,0.7);
  leg_b->SetTextSize(0.035);
  leg_b->SetTextFont(42);
...
void Plots_R2(int mode=0) {
  ...
  c_AFBb->Print("plots_draft/R_acolcut.eps");
