...
void Plots_R_AFB_comparison(bool statonly=false) {
  ...
  double y[8]={r_stat_noTPC[0],r_stat_noTPC[1],afb_stat_noTPC[0],afb_stat_noTPC[1],r_stat_noTPC[2],r_stat_noTPC[3],afb_stat_noTPC[2],afb_stat_noTPC[3]};
  double x[8]={0.5,1.5,3.5,4.5,6.5,7.5,9.5,10.5};
  ...
  double y2[4]={sqrt(pow(afb_stat_dEdx[0]/sqrt(0.35),2)+pow(afb_syst[0],2)),sqrt(pow(afb_stat_dEdx[1]/sqrt(0.35),2)+pow(afb_syst[1],2)),sqrt(pow(afb_stat_dEdx[2]/sqrt(0.65),2)+pow(afb_syst[2],2)),sqrt(pow(afb_stat_dEdx[3]/sqrt(0.65),2)+pow(afb_syst[2],2))};
  double x2[4]={3.5,4.5,9.5,10.5};

  double ex2[4]={0,0,0,0};
  double ey2[4]={y2[0]-sqrt( pow(afb_stat_dEdx[0],2)+pow(afb_syst[0],2)),y2[1],y2[2]-sqrt( pow(afb_stat_dEdx[2],2)+pow(afb_syst[2],2)),y2[3]};//0.5,0.5,0.5,0.5};
  TGraphAsymmErrors * g_noTPC = new TGraphAsymmErrors(4,x2,y2,ex2,ex2,ey2,ex2);

  cout<<"noTPC"<<endl;//AFB no TPC
  ...
  //ILD noTPC
  c_eL_noTPC->Fill(0.5, sqrt( pow(r_stat_noTPC[0],2)+pow(r_syst[0],2)));
  c_eL_noTPC->Fill(3.5, y2[0] );

  c_eR_noTPC->Fill(1.5, sqrt( pow(r_stat_noTPC[1],2)+pow(r_syst[1],2)));
  c_eR_noTPC->Fill(4.5, y2[1] );

  b_eL_noTPC->Fill(6.5, sqrt( pow(r_stat_noTPC[2],2)+pow(r_syst[2],2)));
  b_eL_noTPC->Fill(9.5, y2[2] );

  b_eR_noTPC->Fill(7.5, sqrt( pow(r_stat_noTPC[3],2)+pow(r_syst[3],2)));
 ...
 //
  ...
  //    QQBARLabel2(0.1,0.05, "e^{-}e^{+} #rightarrow "+quark,color+2);
  QQBARLabel(0.89,0.954,"");
  QQBARLabel2(0.15,0.96, "ILC500 4000 fb^{-1} ",960);
