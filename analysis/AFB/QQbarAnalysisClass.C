...
void QQbarAnalysisClass::AFB_histos_for_PQ_analysis(int n_entries=-1, int bkg=0, float Kvcut=35, int quark=4)
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    float costheta_jet_charged_pfos=fabs(GetCostheta(p1));
    
    h_Ntotal_nocuts->Fill(costheta_jet_charged_pfos);
    h_Ntotal_nocuts2[iquark]->Fill(costheta_jet_charged_pfos);
    ...
    costheta_jet_charged_pfos=fabs(costheta_jet_charged_pfos)*costheta_ccbar/fabs(costheta_ccbar);
    ...
    //analysis
    h_N0tag[iquark]->Fill(fabs(costheta_jet_charged_pfos));  
    h_Nq[iquark]->Fill(fabs(costheta_jet_charged_pfos));
    h_Nparton[iquark]->Fill(fabs(costheta_jet_charged_pfos));
    if(bkg==0)h_AFB[iquark]->Fill(costheta_jet_charged_pfos);
    ...
    if(jettag[0]==true)  h_N1tag[iquark]->Fill(fabs(costheta_jet_charged_pfos));
    if(jettag[1]==true)  h_N1tag[iquark]->Fill(fabs(costheta_jet_charged_pfos));
    
    if(jettag[0]==false || jettag[1]==false) continue;
    h_N2tag[iquark]->Fill(fabs(costheta_jet_charged_pfos));
    ...
    for(int i=0; i<5; i++) {
      //Efficiencies
      h_N0[iquark][i]->Fill(fabs(costheta_jet_charged_pfos));
      for(int ijet=0; ijet<2; ijet++) {
	if(charge[ijet][i]!=0) {
	  h_Charge[iquark][i]->Fill(charge[ijet][i]);
	  h_N1[iquark][i]->Fill(fabs(costheta_jet_charged_pfos));
	}
      }
      if(charge[0][i]!=0 && charge[1][i]!=0) h_N2[iquark][i]->Fill(fabs(costheta_jet_charged_pfos));

      float costheta_temp=  (charge[0][i] < 0) ? -costheta_jet: costheta_jet;

      //purities
      if(charge[0][i]*charge[1][i]<0) h_Nacc[iquark][i]->Fill(fabs(costheta_jet_charged_pfos)*costheta_temp/fabs(costheta_temp));
      if(charge[0][i]*charge[1][i]>0) {
	h_Nrej[iquark][i]->Fill(costheta_jet_charged_pfos);
	h_Nrej[iquark][i]->Fill(-costheta_jet_charged_pfos);
      }
    }
...
//variations for systematics
  ...
  TH1F *h_AFB_detector = new TH1F("h_AFB_detector","h_AFB_detector",40,-1.0,1.0);

  TH1F *h_AFBcheat = new TH1F("h_AFBcheat","h_AFBcheat",40,-1.0,1.0);
  TH1F *h_samesign = new TH1F("h_samesign","h_samesign",40,-1.0,1.0);

  TH1F *h_AFBreco[50];
  TH1F *h_AFBreco_pqcorrected[50];
  TH1F *h_AFBreco_chargecheat[50];
  TH1F *h_AFBreco_pqcorrected_effcorrected[50];
  TH1F *h_AFBreco_chargecheat_effcorrected[50];

  for(int i=0; i<50; i++) {
    h_AFBreco[i] = new TH1F(TString::Format("h_AFBreco_%i",i),TString::Format("h_AFBreco_%i",i),40,-1.0,1.0);
    h_AFBreco_pqcorrected[i]= new TH1F(TString::Format("h_AFBreco_pqcorrectedreco_%i",i),TString::Format("h_AFBreco_pqcorrected_%i",i),40,-1.0,1.0);
    h_AFBreco_chargecheat[i]= new TH1F(TString::Format("h_AFBreco_chargecheatreco_%i",i),TString::Format("h_AFBreco_chargecheat_%i",i),40,-1.0,1.0);
    h_AFBreco_pqcorrected_effcorrected[i]= new TH1F(TString::Format("h_AFBreco_pqcorrectedreco_effcorrected_%i",i),TString::Format("h_AFBreco_pqcorrected_effcorrected_%i",i),40,-1.0,1.0);
    h_AFBreco_chargecheat_effcorrected[i]= new TH1F(TString::Format("h_AFBreco_chargecheatreco_effcorrected_%i",i),TString::Format("h_AFBreco_chargecheat_effcorrected_%i",i),40,-1.0,1.0);
  }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    p1.push_back(py_pfos[0]-py_pfos[1]);
    p1.push_back(pz_pfos[0]-pz_pfos[1]);
    float costheta_jet_charged_pfos=GetCostheta(p1);

    int abs_value_costheta_bin = int(fabs(costheta_jet_charged_pfos)*100./5);
    ...
    h_AFB->Fill(costheta_cheat);
    h_AFB_detector->Fill(fabs(costheta_jet_charged_pfos)*costheta_cheat/fabs(costheta_cheat));
	...
	if(method==0) charge[ijet][0]=ChargeKJet(0,ijet,pcut,false,false);
	if(method==1) charge[ijet][0]=ChargeKJet(0,ijet,pcut,true,false);
	if(method==2) charge[ijet][0]=ChargeKJet(0,ijet,pcut,true,true);
	if(method==3) charge[ijet][0]=ChargeKJet(0,ijet,pcut,false,true);
      }
    }
    ...
    for(int isyst=0; isyst<nsyst; isyst++) {
      ...
      float eff_tag=eff_tagging[isyst]->GetBinContent(abs_value_costheta_bin+1)*sqrt(1+rhovalue);
	...
	if(charge[ijet][1]!=0 && charge[ijet][0]==0) {
      ...
      float prob_notBc=1.;
      if(jet_charge[0]*jet_charge[1]<0 && charge[0][0]!=0 && charge[1][0]!=0 )  {
	double p1=prob_charge[0];
	double p2=prob_charge[1];
	double q1=1-p1;
	double q2=1-p2;
	double weightpq=1./(p1*p2-q1*q2);
	double weighteff=1./(3.*prob_notBc*eff_tag*eff_tag*eff_charge[0]*eff_charge[1]*(p1*p2+q1*q2));
	float costheta_reco=  (jet_charge[0] < 0) ? -costheta_jet_charged_pfos: costheta_jet_charged_pfos;
	...
	h_AFBreco[isyst]->Fill(costheta_reco);
	h_AFBreco_pqcorrected[isyst]->Fill(costheta_reco,prob_plus);
	h_AFBreco_pqcorrected[isyst]->Fill(-costheta_reco,prob_minus);
	h_AFBreco_pqcorrected_effcorrected[isyst]->Fill(costheta_reco,prob_plus*weighteff);
	h_AFBreco_pqcorrected_effcorrected[isyst]->Fill(-costheta_reco,prob_minus*weighteff);
	
	h_AFBreco_chargecheat[isyst]->Fill(fabs(costheta_reco)*costheta_cheat/fabs(costheta_cheat));
	h_AFBreco_chargecheat_effcorrected[isyst]->Fill(fabs(costheta_reco)*costheta_cheat/fabs(costheta_cheat),weighteff);
	...
	if(jet_charge[0]*jet_charge[1]<0 && charge[0][0]==0 && charge[1][0]==0 )  {
	  ...
	  if(jet_charge[0]*jet_charge[1]<0 && (charge[0][0]==0 ||  charge[1][0]==0) &&  (charge[0][0]!=0 ||  charge[1][0]!=0) ) {
	    prob_notBc=2.;
double p1=prob_charge[0];
	    double p2=prob_charge[1];
	    double q1=1-p1;
  ...
  MyFile->cd();
  h_AFB->Write();
  h_AFB_detector->Write();

  h_AFBcheat->Write();
  h_samesign ->Write();
  
  for(int isyst=0; isyst<nsyst; isyst++) {
    TF1* func = new TF1("func","[0]+[1]*x+[2]*x*x+[3]*x*x*x",0.0,0.85);
    ...
    for(int j=0; j<40; j++) {
      int abs_value_costheta_bin = int(fabs(h_AFBreco_pqcorrected[isyst]->GetBinCenter(j+1)*100./5));
      float eff_pres_=eff_preselection[isyst]->GetBinContent(abs_value_costheta_bin+1);
      if(isyst==(nsyst-1))  {
	float x=eff_preselection[isyst]->GetBinCenter(j+1);
	eff_pres_=a+x*b+x*x*c+x*x*x*d;
      }
      if(eff_pres_>0) 	{
	h_AFBreco_pqcorrected_effcorrected[isyst]->SetBinContent(j+1, h_AFBreco_pqcorrected_effcorrected[isyst]->GetBinContent(j+1)/eff_pres_);
	h_AFBreco_chargecheat_effcorrected[isyst]->SetBinContent(j+1, h_AFBreco_chargecheat_effcorrected[isyst]->GetBinContent(j+1)/eff_pres_);
      }
      
    }
    
    h_AFBreco[isyst]->Write();
    h_AFBreco_pqcorrected[isyst]->Write();
    h_AFBreco_chargecheat[isyst]->Write();
    h_AFBreco_pqcorrected_effcorrected[isyst]->Write();
    h_AFBreco_chargecheat_effcorrected[isyst]->Write();
  }
