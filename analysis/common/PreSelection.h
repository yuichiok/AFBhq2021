...
float Kreco()
{
  ...
  */
  ...
  if (n_charged_pfos[0] < 2 || n_charged_pfos[1] < 2)
    Kv = 1000;
  return Kv;
}
...
bool PreSelection(int type = 0, float acolcut = 0.3)
{
  ...
  cut_[5] = (cut_[4] && principle_thrust_value>0.95);//d23 > 0.5 && d23 / pow(bbmass, 2) < y23cut);

  cut_[6] = (cut_[5] && jet_btag[0] > btag1 && jet_btag[1] > btag2);
  cut_[7] = (cut_[6] && jet_ctag[0] > ctag1 && jet_ctag[1] > ctag2);
  return cut_[type];
}
